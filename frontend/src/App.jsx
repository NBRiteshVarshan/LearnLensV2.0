...
async function generateSummary(pdfId, userId) {
  return apiFetch("/summary", {
    method: "POST",
    body: JSON.stringify({ pdf_id: pdfId, user_id: userId }),
    headers: { "Content-Type": "application/json" },
  });
}
...
export default function App() {
  ...
  const handleSummary = async () => {
    ...
    const res = await generateSummary(selectedPdf.id, userId);
  ...
  return (
    <div style={{ padding: 20 }}>
      ...
      <div>
        {pdfs.map((p) => (
          <div key={p.id} onClick={() => setSelectedPdf(p)}>
            📄 {p.name}
          </div>
        ))}
      </div>
      ...
      <div>
        <h2>Summary</h2>
        <Btn onClick={handleSummary}>Generate</Btn>
        <p>{summary}</p>
      </div>
      ...
      <div>
        <h2>Quiz</h2>
        <Btn onClick={handleQuiz}>Generate Quiz</Btn>
        ...
        {quiz &&
          quiz.map((q, i) => (
            <div key={i}>
              ...
              <Btn onClick={() => checkAnswer(i)}>Check</Btn>
            </div>
          ))}
